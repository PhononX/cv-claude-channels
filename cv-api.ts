...
// The signal endpoint (POST /v5/conversations/:id/signal) broadcasts an ephemeral
// activity indicator to clients who have the conversation open. It is stateless:
// nothing is stored server-side, so a signal must be re-sent every ~1-2s to stay
// visible and simply stops being sent to clear it. ttl_ms is a client-side expiry
// hint only. is_agent is stamped server-side from the PAT identity.
